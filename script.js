...
function addMessage(sender, text, save = true) {
    ...
    if (sender === 'bot') {
        ...
        icon.src = 'image/icon.jpg'; // Path to your icon
        icon.alt = 'AI Icon';
    ...
    if (sender === 'bot') {
        ...
        // Code block highlighting and copy button (inside bubble)
        const codeBlocks = messageBubble.querySelectorAll('pre');
        codeBlocks.forEach(pre => {
            const codeElement = pre.querySelector('code');
            if (codeElement) hljs.highlightElement(codeElement);
            else hljs.highlightElement(pre);
            // Add code copy button (logic unchanged, stays inside pre)
    ...
    copyMessageButton.addEventListener('click', (e) => {
        ...
        navigator.clipboard.writeText(textToCopy).then(() => {
            // Provide feedback (e.g., temporarily add text or change icon)
            const feedbackSpan = document.createElement('span');
            feedbackSpan.textContent = '已复制!';
            feedbackSpan.className = 'copy-feedback';
            copyMessageButton.appendChild(feedbackSpan);
            copyImg.style.display = 'none'; // Hide icon during feedback

            setTimeout(() => {
                copyMessageButton.removeChild(feedbackSpan); // Remove feedback
                copyImg.style.display = 'inline'; // Show icon again
            }, 1500);
        }).catch(err => {
